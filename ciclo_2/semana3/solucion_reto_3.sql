...
SELECT p.Ciudad, p.Clasificacion, COUNT(*) AS TOTAL, MIN(p.Fecha_Inicio) AS VIEJO, MAX(p.Fecha_Inicio) AS RECIENTE 
FROM Proyecto AS p
...
JOIN MaterialConstruccion AS mc ON(mc.ID_MaterialConstruccion = c.ID_MaterialConstruccion)
WHERE c.Pagado = 'No'
...
FROM Lider AS l
JOIN Proyecto AS p ON(p.ID_Lider = l.ID_Lider)
JOIN Compra AS c ON(c.ID_Proyecto = p.ID_Proyecto)
